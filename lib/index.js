...
NDoc.prototype.toHTML = function (options) {
  ...
  function link(obj, short, classes, area) {
    ...
    if (dotPos < 0 && obj.id.indexOf("new") < 0 && atPos < 0)
    {
      ...
      {
        var r = '<a href="' + obj.parentDir + "/" + obj.id + '.html"' + '" class="' + (classes || []).join(' ')
          + '" title="' + obj.id + (obj.type ? ' (' + obj.type + ')' : '') + '">' + obj.id + '</a>';
      }
      else  // for the toc menu
      {
        var r = '<a href="' + obj.parentDir + "/" + obj.id + '.html" id="' + obj.id + '" class="' + (classes || []).join(' ')
    ...
    else // for non-methods (constants, events, constructors)
    {
      if (obj.name == undefined)
        ...
        var r = '<a href="' + obj.parentDir + "/" + filename + '.html#' + obj.path
          + '" class="' + (classes || []).join(' ')
