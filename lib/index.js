...
function NDoc(files, options) {
  ...
  files.forEach(function (file) {
    ...
    TODO: for dirs with versions--e.g., src/ref/v1, src/ref/v2
   ...
   /* if (file.indexOf("/v") >= 0) 
    {
      parentDir = parentDir[1] + "/" + parentDir[2];
    }
    else
    {
    ...
    try {
      ...
      for (id in ndocs) {
        if (ndocs.hasOwnProperty(id)) {
          ...
          d.filename = file.substring(file.lastIndexOf("/") + 1);
          if (d.type === 'class') {
            d.subclasses = [];
          }
          // collect sections
...
NDoc.prototype.toHTML = function (options) {
  ...
  function link(obj, short, classes, area) {
    ...
    if (dotPos < 0 && obj.id.indexOf("new") < 0 && atPos < 0)
    {
      ...
      {
        var r = '<a href="' + obj.id + '.html"' + '" class="' + (classes || []).join(' ')
          + '" title="' + obj.id + (obj.type ? ' (' + obj.type + ')' : '') + '">' + obj.id + '</a>';
      }
      else  // for the toc menu
      {
        ...
        var r = '<a href="' + obj.id + '.html" id="' + obj.id + '" class="' + (classes || []).join(' ')
          + ' clicker" title="' + obj.id + (obj.type ? ' (' + obj.type + ')' : '')
          + '" data-id="' + obj.id + '">';
    ...
    else // for non-methods (constants, events, constructors); also for menu tabs up top
    {
      ...
      if (obj.name == undefined) {
        ...
        if (dotPos >= 0 ) {
          ...
          if (numOfDots > 1) // usually for subchildren, e.g. http.ServerResponse.addTrailers
            filename = names[0] + "." + names[1];
          else
            filename = obj.id.substring(0, dotPos);
        }         
        else
          filename = obj.path.substring(0, obj.path.indexOf('.'));

        if (obj.path === undefined)
          obj.path = ""; // removes 'undefined' from window url
          
        var r = '<a href="' + filename.replace(/_/g,'.').replace(/new /g,'') + '.html#' + obj.path
          + '" class="' + (classes || []).join(' ')
  ...
  function signature(obj, sig, type, id) {
    ...
    if (obj.id.indexOf("new") >= 0)
      r = 'new <span id="' + id + '" class="member-name methodClicker">' + obj.id.substring(obj.id.indexOf(" ") + 1) + '</span>';
    ...
    else if (type == "event")
    {
      ...
      var eventName = obj.id.substring(obj.id.lastIndexOf("."));
      r = '<span class="eventObjName">' + obj.id.substring(0, obj.id.indexOf(".event")) +'</span><span class="eventListenerStart">' + '.on(</span>\"<span id="' + id + '" class="member-name methodClicker eventMember">' + eventName + '</span>\"'; 
    }
    ...
    if (sig.args) {
      if (type != 'event')
        ...
        r += ", <span class='eventFunctionOpen'>function(</span>"
      ...
      sig.args.forEach(function (arg, idx, args) {
        ...
        if (obj.arguments && obj.arguments[idx])
        {
          var link = "";
          obj.arguments[idx].types.forEach(function (arg, idx, args) {
              var argLink = arg;
              /*if (arg.indexOf(".") >= 0)
              {
                var dotPos = arg.indexOf(".");
                argLink = arg.substring(0, dotPos) + "#" + arg.substring(dotPos + 1);
              }*/

              link += '<a href="' + argLink + '.html">' + arg + '</a>';
